...
def Analysis_Gate():
    if radio_string.get() == "un":  # One Sample
        ...
        Analysis_Room(adata, percent_mt=5, min_genes=150,min_cells=3)  # je lance la vraie analyse (pour l'instant je code l'analyse rapide mais aprés faudra passer des paramétres pour l'analyse détaillé)
        ...
        Analysis_Room(adata, percent_mt=5, min_genes=150, min_cells=3)  # je lance la vraie analyse , avec un adata unifié !!
...
ok_count_button_tab1 = ttk.Button(tab1, text="OKEY", command=lambda: check_path(tab1)).place(x=280, y=13) # j'ai utiliser .place mais je vais peut-être le regretter
